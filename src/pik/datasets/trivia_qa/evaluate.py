...
def evaluate_answer(
    ...
    if isinstance(model_answers, str) and not isinstance(dataset_answers[0], str):
        raise ValueError(
            "If model_answers is str, dataset_answers expects list[str]."
        )
    ...
    results = []
    for model_answer, aliases in zip(model_answers, dataset_answers):
        model_answer = normalize_answer(model_answer)
        if any(alias in model_answer for alias in aliases):
            results.append(1)
        else:
            results.append(0)
